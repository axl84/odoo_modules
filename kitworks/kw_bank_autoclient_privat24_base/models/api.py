...
class Privat24AutoclientApi():
    ...
    def request_url(self, type_request='data', **arg):
        ...
        r = requests.get(url, params=arg, headers=self.HEADERS, timeout=30)
    ...
    def create_payment(self, **kwargs):
        ...
        filtered_kwargs = {
            key: val for key, val in kwargs.items() if key in PAYMENT_FIELDS
        }

        for x in filtered_kwargs.keys():
            kwargs.pop(x)

        return self.request_url(
            type_request='payment/create_pred',
            body=json.dumps(filtered_kwargs),
        )
