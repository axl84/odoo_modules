...
class Conversation(models.Model):
    ...
    def helpcrunch_update_contact(self, text, text_resp):
    ...
    def helpcrunch_get_response(self, jsonrequest):
        if self.dialog_id.bots_type == 'is_consultant_bot':
            return super().helpcrunch_get_response(jsonrequest)

        text = jsonrequest['message']['text']

        # self.is_helpcrunch_send = False
        ...
        if not trig_step and not self.wired_conversation_id:
            if not self.dialog_id.wait_start_message and text != '/end':
                _logger.info(
                    'я не знайшов стартового кроку і змінюю на /start')
                trig_step = self.env['kw.chatbot.step'].sudo().search([
        ...
        if self.input_step_id:
            ...
            if self.input_step_id.step_type == 'update_contact_field':
                ...
                self.helpcrunch_update_contact(text=text, text_resp=text_resp)
            ...
            self.helpcrunch_next_step(self.last_step_id, text)
            return True

        if trig_step:
            _logger.info('33 conv')
            if trig_step.helpcrunch_get_response(
                    conversation=self, message=text):
                self.write({
                    'is_helpcrunch_send': True,
            ...
            # if we are expecting input
            if not self.input_step_id:
                self.helpcrunch_next_step(trig_step, text)
        ...
        if text == '/end' and self.last_step_id:
            self.helpcrunch_next_step(self.last_step_id, text)
            return True
        res = super().helpcrunch_get_response(jsonrequest)
        if not self.is_helpcrunch_send:
            self.send_message(
                text=(self.dialog_id.not_found_msg.strip()))
        return res

    def helpcrunch_next_step(self, step, text, next_step=None):
        if not next_step:
            next_step = step.go_to_step_id
        _logger.info('next_step %s', next_step.name)
        for obj in next_step:
            if obj.helpcrunch_get_response(
                    conversation=self, message=text):
    ...
    def create(self, vals_list):
        if vals_list.get('text') == '/end' \
            ...
            if conversation_id.wired_conversation_id:
                ...
                if conv_id.chat_id.provider == 'help_crunch' \
                        and conv_id.last_step_id \
                        and conv_id.last_step_id.go_to_step_id:
                    conv_id.helpcrunch_next_step(
