...
class Step(models.Model):
    ...
    def echat_get_response(self, conversation, message):
        _logger.info('echat_get_response')
        self.ensure_one()
        ...
        if conversation.sender_id.partner_id:
            ...
            if conversation.sender_id.partner_id not in check_partner:
                ...
                step.echat_get_response(conversation, message)
                if not conversation.input_step_id:
                    conversation.echat_next_step(step=step, text=message)
                return False
        ...
        if self.step_type == 'create_lead':
            ...
            hc_channel_name = 'E-Chat ({})'.format(
                conversation.chat_id.echat_mobile_phone)
