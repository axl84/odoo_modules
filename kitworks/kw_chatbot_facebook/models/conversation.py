...
class Conversation(models.Model):
    ...
    is_facebook_send = fields.Boolean()

    def send_message(self, text, **kwargs):
        self.ensure_one()
        if self.chat_id.messenger_id.provider != 'facebook':
            return super().send_message(text, **kwargs)
        bot = self.chat_id.facebook_get_facebook_bot()
        markup = kwargs.get('markup')
        self.is_facebook_send = True
        if not self.wired_conversation_id:
            data = {'text': text}
            if markup:
                data['buttons'] = [i['title'] for i in markup]
            self.facebook_out_message(text=data)
        if markup:
            while markup:
                m = markup[:3]
                markup = markup[3:]
                res = bot.send_button_message(
                    self.sender_id.facebook_id, text, m)
                self.facebook_create_log(name='OUT', body=res)
        else:
            res = bot.send_text_message(
                self.sender_id.facebook_id, text)
            self.facebook_create_log(name='OUT', body=res)
        return None

    # pylint: disable=R1711
    def facebook_out_message(self, text=False):
        self.ensure_one()
        if not self.wired_conversation_id and not self.operator_live_id:
            sender = self.env['kw.chatbot.sender'].search([
                ('name', '=', 'FacebookBot')], limit=1)
    ...
    def facebook_create_log(self, name=False, body=False):
        self.ensure_one()
    ...
    def facebook_get_message_data(self, message):
    ...
    def get_or_create(self, chat, **kwargs):
        if chat.messenger_id.provider != 'facebook':
    ...
    def facebook_get_response(self, bot, message):
        ...
        if self.dialog_id.bots_type == 'is_consultant_bot' \
                and self.dialog_id.user_ids:
            message = self.facebook_get_message_data(message)
            live_chat_mess_id = self.connect_live_chat(message)
            if live_chat_mess_id:
                self.write({'is_facebook_send': True})
        if not self.is_facebook_send:
            self.send_message(self.dialog_id.not_found_msg)
        return None

    def facebook_utm_data(self, index):
        self.ensure_one()
        if index:
