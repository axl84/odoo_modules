...
from .constants import TransactionType, API_URL
...
class Api:
    ...
    def _query(self, transaction_type: TransactionType, params: dict):
        params = FrozenParams(
            self.merchant_account, self.merchant_key, transaction_type, params)
        response = requests.post(API_URL, json=dict(params), timeout=30)
