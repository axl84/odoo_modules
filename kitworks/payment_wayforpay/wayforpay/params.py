...
from .constants import TransactionType, SIGNATURE_FIELDS, REQUIRED_FIELDS, \
    ChargeTransactionType, SUPPORTED_LANGUAGES, PAYMENT_SYSTEMS
...
class Params(ParamsBase):
    validators = {
        **{field: _val_not_empty_validator for field in set(
            chain.from_iterable(REQUIRED_FIELDS.values()))},
    ...
    def get_required_fields(self) -> set:
        ...
        if key is TransactionType.CHARGE:
            key = ChargeTransactionType.REC_TOKEN if self.get('recToken') \
                else ChargeTransactionType.NO_REC_TOKEN
    ...
    def _generate_signature(self, merchant_key: str):
        ...
        return generate_signature(merchant_key, [self[field] for field
                                                 in signature_fields])
...
class FrozenParams(ParamsBase):
    def __init__(self, merchant_account, merchant_key: str,
                 transaction_type: TransactionType, params: dict):
        super().__init__()
        self._store = Params(transactionType=transaction_type.value,
                             merchantAccount=merchant_account, **params)
